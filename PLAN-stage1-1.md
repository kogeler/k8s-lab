...
выполненные phases (§14). Stage 1 closure + self-review + final
recommendation живут в PLAN-stage1-7.md (§20..§22). Нумерация §N
...
* `bootstrap_capn_secret_pivot_enabled` defaults `true` — pivot
  mandatory в canonical flow §3, label
  `clusterctl.cluster.x-k8s.io/move=true` всегда пробрасывается, чтобы
  `clusterctl move` тащил Secret на target mgmt-1;
...
path на custom-образ в consumer-overlay не требует рефакторинга
bootstrap-path.
...
* **K8s Secret apply — fanout по namespace'ам workload Cluster
  ...
  дефолту (canonical flow §3, pivot mandatory), чтобы `clusterctl
  move` тащил Secret на target mgmt-1.
...
* **Idempotence на edge-кейсах.**
  ...
  - Secret apply (per target ns): server-side apply + byte-stable
    ...
    flip via `bootstrap_capn_secret_pivot_enabled` распространяется
    чисто (server-side apply field-manager корректно стомпит/убирает
...
* **Public defaults — sourced from §8 contract:**
  ...
  - `bootstrap_capn_secret_pivot_enabled` — defaults `true`
    (canonical flow §3 pivot mandatory). Override на `false` только
    для ad-hoc substrate-only test runs;
...
* Secret(s): для каждой ns из `k8s_lab_capn_identity_namespaces`
  ...
  required ключами + `project = k8s_lab_project_name`; pivot label
  `clusterctl.cluster.x-k8s.io/move=true` присутствует по дефолту
...
`kubectl --kubeconfig=.artifacts/mgmt.kubeconfig get nodes` с
dev-машины возвращает Ready control-plane node.**

Закрывает Phase 4: ship'ит handoff bundle с LXD host'а на runner в
`.artifacts/`:

* **`.artifacts/mgmt.kubeconfig`** — admin kubeconfig для активного
  management-кластера. На первом include'е роли (через meta-chain)
  файл указывает на bootstrap k3s — источник host-side
  `/opt/capi-lab/etc/bootstrap_clusterctl/bootstrap.kubeconfig`
  (материализованный `bootstrap_clusterctl` с уже переписанным
  server URL под container-eth0 IPv4 и `tls-server-name:
  kubernetes.default.svc`). На втором include'е роли с
  `export_artifacts_run_meta_chain: false` + source-override на
  pivot host-side staging — тот же runner-side файл
  перезаписывается in place mgmt-1 creds'ами (canonical flow §3).
  Runner-side потребители (TF workload fixtures, Molecule e2e-local)
  держат один путь через всю lifecycle.
* **`.artifacts/mgmt.auto.tfvars.json`** — TF-native `*.auto.tfvars.json`
  ...
  + производные `k8s_lab_mgmt_kubeconfig_path` и
  `k8s_lab_mgmt_api_server_url`. API URL derived из shipped
...
* **Runner-reach через LXD proxy + `tls-server-name`.** Runner
  (dev-машина для harness'а, сервер для production) должен
  достучаться до mgmt API, но в cert'е k3s нет SAN для IP host'а.
  Решено через:
  - `bootstrap_clusterctl` pin'ит `tls-server-name: kubernetes.default.svc`
    (стандартный k8s SAN, всегда в k3s cert) — см. §13.10;
...
* **Substrate-required в `vars/main.yml`** (правило памяти
  ...
  - `_export_artifacts_required_mgmt_kubeconfig_filename: "mgmt.kubeconfig"`
    — Phase 5 fixture'ы хардкодят именно это имя; тот же путь
    перезаписывается in place при pivot (§3.1 / §18 — отдельной
    `bootstrap.kubeconfig` сущности нет);
...
* **Public defaults — tunable:** whole-role toggle
  ...
  на host'е (`export_artifacts_mgmt_kubeconfig_source`,
  default'ом указывает на host-side
  `/opt/capi-lab/etc/bootstrap_clusterctl/bootstrap.kubeconfig` —
  internal staging file роли `bootstrap_clusterctl`, не runner-side
  имя),
...
* **Healthchecks** stat'ят оба файла на runner'е, проверяют mode 0600
  ...
  + что `k8s_lab_mgmt_api_server_url` начинается с `https://` и
...
### Adjacent debug artefacts (вне scope этой роли)

* **`.artifacts/clusters/<cluster>.kubeconfig`** — runner-reachable
  rewritten kubeconfig'и для mgmt-1 и workload, написанные
  e2e-local converge.yml (§10.2) парсингом kubeconfig Secret'ов на
  bootstrap'е/mgmt-1 и подменой server URL на
  `https://<lxd_host>:<api-proxy-port>` + `tls-server-name`. TF
  module §16.4 в этот subdir не пишет (§16.4 architectural fence);
  module держит rewritten kubeconfig в TF state и эмитит через
  `terraform output -raw kubeconfig`. Subdir создаётся ролью
  `export_artifacts` на её первом include'е.
...
defaults `true` (canonical flow §3, pivot mandatory). Single global
flip держит Phase 5+ Cluster CR identityRef и pivot move-label синхронизированными
...
* `.artifacts/mgmt.kubeconfig` — через `slurp`+`copy` с
  `delegate_to: localhost, become: false, run_once: true` (контракт
  §11.1: file mode 0600, owner=runner user). Опциональный rewrite
  `clusters[].cluster.server` через публичную `export_artifacts_mgmt_api_server_url`
  (empty → keep host-side URL; non-empty → подставить в shipped
  kubeconfig). `tls-server-name` сохраняется `as-is` из source —
  bootstrap_clusterctl уже pin'ит его. После pivot'а тот же файл
  перезаписывается in place вторым include'ом роли с
  `export_artifacts_run_meta_chain: false` + source-override на
...
* `.artifacts/mgmt.auto.tfvars.json` — TF-native auto-load
  ...
  etc.) + производные `k8s_lab_mgmt_kubeconfig_path`,
  `k8s_lab_mgmt_api_server_url` (деривится из уже shipped
  kubeconfig'а — второй LXD REST probe не нужен).
* `.artifacts/clusters/` — пустой subdir, зарезервирован для
  per-workload debug-копий (Molecule e2e-local verify пишет туда
  raw kubeconfig для operator inspection).

Substrate-required значения (filenames `mgmt.kubeconfig` /
`mgmt.auto.tfvars.json`, subdir `clusters/`, file mode 0600 / dir
mode 0700) — в `vars/main.yml` под `_export_artifacts_required_*`
префиксом. Public defaults экспонируют только toggles, source path
на host'е, `tfvars_extra` merge-on-top точку расширения и
`export_artifacts_mgmt_api_server_url` для runner-reach override'а;
`export_artifacts_root` — обязательный consumer input (plan §11.1
фиксирует контракт, не путь; preflight режёт `..`/`.` сегменты для
канонической формы). Meta-deps: `bootstrap_capn_secret` (закрывает
Phase 4 substrate chain — транзитивно всё до `base_system`).
...
node возвращается. `kubectl --kubeconfig=.artifacts/mgmt.kubeconfig
...
* `bootstrap_capn_secret` (см. §13.11) материализует CAPN identity
  ...
  всегда добавляется (`bootstrap_capn_secret_pivot_enabled` defaults
...
* bootstrap_capn_secret scenario: LXD `core.https_address` bound на
  ...
  `clusterctl.cluster.x-k8s.io/move=true` присутствует
  (canonical flow §3 default).

Сделано в Step 8:

* `export_artifacts` (см. §13.12) закрывает runner-side handoff:
  `.artifacts/mgmt.kubeconfig` (shipped с host'а на runner, mode
  0600 по §11.1; опциональный rewrite `clusters[].cluster.server` на
  runner-reachable URL через `export_artifacts_mgmt_api_server_url`)
  + `.artifacts/mgmt.auto.tfvars.json` (§8 globals зеркалены 1:1,
  Terraform auto-load'ит файл в Phase 5 fixture root'ах).
  `.artifacts/clusters/` subdir создан для per-workload debug-копий.
...
* tfvars парсится как JSON, содержит baseline `k8s_lab_*` ключи,
  `k8s_lab_mgmt_api_server_url` совпадает с
...
* Test fixture root `tests/fixtures/terraform/workload-clusters/lab-default/` —
  ...
  из `k8s_lab_mgmt_api_server_url` host-component через regex
...
Acceptance workload TF route — **закрыта** (2026-04-28):
...
**Статус: реализован в Step 18 (2026-04-29). См. §18
(PLAN-stage1-5.md) для полного контракта (mgmt-1 helm install
...
* **e2e-local Molecule scenario** `tests/molecule/e2e-local/`
  inlines pivot как обязательную стадию: `pivot_clusterctl_move`
  → второй include `export_artifacts` (re-emit, `run_meta_chain:
  false`) → `cleanup_bootstrap`. Acceptance проверяется
  role's own healthchecks плюс post-pivot workload helm tests
  в verify.yml.
...
Acceptance pivot stage — **зелёный end-to-end** (2026-04-29):

* mgmt-1 helm install зелёный (cluster-class + capi-workload-cluster
  с mgmt-topology values)                            ✓ (Step 18)
...
Post-pivot workload creation on self-hosted mgmt-1 happens inline в
e2e-local converge (canonical flow §3 / §10.2). Дополнительные
workload'ы поверх существующего mgmt-1 — через `make deploy-workload`
(TF route, §16.6) против `.artifacts/mgmt.kubeconfig` (теперь
указывает на mgmt-1) без override'ов.
