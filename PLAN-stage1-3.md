...
в topology-режиме). Per-cluster ConfigMap/Secret'ы для custom
...
annotations:
  k8s-lab.io/capi-cluster-class-chart-version: "0.1.0"
```
...
Rotation contract: bump cluster-class Chart.version `0.1.0 → 0.2.0`
требует парного bump'а annotation **и** workload-cluster Chart.version
(traceability). Consumer (Terraform fixture или standalone
`helm install`) **не передаёт** cluster-class version — coupling
объявлен в Chart.yaml, не в values.yaml.
...
kubernetes:
  version: "v1.36.0"               # chart's own latest-stable pin
                                   # (verified upstream stable.txt;
                                   # §8 k8s_lab_kubernetes_version
...
* **Phase 1 — bootstrap-side shape (~2 min):**
  * Cluster CR present в `.Release.Namespace`;
  * `kubectl wait Cluster --for=condition=TopologyReconciled=True`
    (CAPI accepted topology + spawned owned objects);
  * owned `KubeadmControlPlane` materialised (label
    `cluster.x-k8s.io/cluster-name=<cluster.name>`);
  * owned `MachineDeployment` materialised;
  * owned `LXCCluster` materialised.
* **Phase 2 — workload-side authoritative «node up» check (~20 min default):**
  * poll до появления `<cluster.name>-kubeconfig` Secret в
    `.Release.Namespace` (KCP controller emits его post-init первой
    CP-ноды);
  * decode Secret → `/tmp/wl.kubeconfig`;
  * connect to workload cluster API через этот kubeconfig;
  * count `Node` объектов:
    * по label `node-role.kubernetes.io/control-plane=` → должно быть
      ≥ `topology.controlPlane.replicas`;
    * по label `!node-role.kubernetes.io/control-plane` → должно быть
      ≥ `topology.workers.replicas`;
