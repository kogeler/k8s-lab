...
* **cloud-init-capability — substrate-required для любого образа**,
  ...
  `charts/capi-cluster-class` доставляет eth1 RA reception baseline
  (sysctl + systemd-networkd drop-in) через `KubeadmConfigSpec.files`
  + `preKubeadmCommands` в обоих kubeadm-template'ах (§16.2 / §16.3) —
  CABPK инлайнит их в user-data `write_files`, и cloud-init на
  каждой CAPN-spawned ноде применяет их на first boot. CAPN-prebuilt
  `capi:kubeadm/*` образы это гарантируют; custom-образ консумера
  без cloud-init'а = неработающий external L2 plane на всех
  worker'ах.
...
где capability впервые реально нужна.

Test stack: chart's hook driver Pod создаёт реальный demo
Deployment (nginx-on-alpine, dual-family `listen 80; listen [::]:80;`)
+ Service type=LoadBalancer (`ipFamilies: [IPv6]`, single-stack v6).
Demo stack driver-managed (kubectl apply изнутри driver Pod'а), не
аннотирован `helm.sh/hook: test` — переживает `Phase: Succeeded` и
доступен verify-side тесту для external curl.

Acceptance criteria (Gate A зелёный когда оба прошли):

1. **chart-side helm test hook PASS** (`helm test metallb-config`,
   exit=0). 8 фаз in-cluster:
   - kubectl install
   ...
   - in-cluster HTTP probe FROM driver Pod (`wget [VIP]:80`,
     non-hairpin path: driver не endpoint Service'а, kube-proxy
...
Test driver Pod survives `Phase: Succeeded` via
...
реального окружения обязан прогнать эквивалентный test на actual
external segment — это уже тот же chart, те же profiles, только RA
приходит от настоящего провайдерского роутера вместо in-VM radvd, и
verify-side curl запускается с действительно-внешнего endpoint'а.
...
## Substrate предусловие — eth1 RA reception baseline через KubeadmConfigSpec

Для того чтобы Gate A/B helm test'ы в §16.4 module вообще имели
работающий eth1 на worker-нод'ах — external nic должен быть
сконфигурирован **до первого Helm install'а** в module chain'е. Эту
baseline доставляет `charts/capi-cluster-class` через
`KubeadmConfigSpec.files` + `preKubeadmCommands` в обоих
`KubeadmControlPlaneTemplate` и `KubeadmConfigTemplate` (§16.2 / §16.3):

* два файла на каждой CAPN-spawned ноде:
  * `/etc/sysctl.d/99-capi-ra.conf` — `net.ipv6.conf.eth1.{disable_ipv6=0,
    accept_ra=2, accept_ra_defrtr=1}`. `accept_ra=2` обязательно потому
    что workload-ноды бегут с `forwarding=1` для k8s pod networking,
    дефолтный `accept_ra=1` ignores RA при forwarding=1;
  * `/etc/systemd/network/30-capi-ext.network` — `[Match] Name=eth1
    [Network] DHCP=no LinkLocalAddressing=ipv6 IPv6AcceptRA=yes`;
* `preKubeadmCommands` apply'ит `sysctl --load` + `networkctl reload`,
  так что config жив до того как kubelet, kube-proxy, MetalLB speaker
  стартуют;
* CABPK инлайнит файлы как user-data `write_files` — единственный
  reliable wiring на kubeadm-bootstrapped node, где CAPI/CABPK
  владеет user-data slot'ом эксклюзивно;
...
Эффект: eth1 SLAAC'ит global IPv6 от upstream RA, MetalLB speaker
посылает NDP replies на announced VIPs с global source IPv6.
...
# Runner-reachable address LXD host'а. Workload kube-apiserver
# слушает на capi-int IPv6 (`fd42:77:1::/64`), который reachable
# только изнутри VM/LXD instances. Module §16.4 пишет workload
# kubeconfig с `server: https://<k8s_lab_lxd_host_address>:<port>`,
# где port — per-workload Adler-32 hash от cluster.name (default
# range 20000-29999, override через chart values
# `loadBalancer.lxc.proxyApiPort`). LXD proxy device на CAPN
# haproxy LB instance с `bind=host` форвардит host:<port> →
# 127.0.0.1:6443 внутри LB. На local Vagrant — Vagrant VM IP, на
# prod — public IP / DNS name LXD host'а.
...
# ---- images ----
# Контракт: оба образа ОБЯЗАНЫ быть cloud-init-capable.
# `charts/capi-cluster-class` доставляет eth1 RA reception baseline
# (sysctl + systemd-networkd drop-in) через `KubeadmConfigSpec.files`
# + `preKubeadmCommands` — CABPK инлайнит их в user-data
# `write_files`, и cloud-init на каждой CAPN-spawned ноде применяет
# их на first boot. CAPN-prebuilt `capi:kubeadm/*` образы это
# гарантируют; custom-образ консумера должен сохранять cloud-init в
# том же виде.
...
(container eth1 получает global IPv6 по SLAAC через RA от
external-сегмента) **никогда не мог пройти end-to-end** против
libvirt-mock — guest's `IPv6AcceptRA=yes` был, принимать было
нечего.
...
parent=br-ext6`. Внутри контейнера `KubeadmConfigSpec.files`
(charts/capi-cluster-class, §16.2 / §16.3) уже выложил
`/etc/systemd/network/30-capi-ext.network` с `IPv6AcceptRA=yes` →
systemd-networkd SLAAC'ит global IPv6 в префиксе.
