...
### 2.6.5. Комментарии, README и проверяемость
...
* исключения фиксируются в `PLAN-stage1-progress.md` как отдельные
  deviation-пункты с обоснованием.
...
* каждая зафиксированная версия и дата проверки фиксируются в
  `PLAN-stage1-progress.md` в таблице «Версии зафиксированы», чтобы
  отличать осознанный pin от устаревшего;
* conflict resolution: если vendor official guidance (например, LXD
  ...
  наиболее свежий feature-stable и явно фиксируем trade-off в
  progress-документе;
* единственное исключение: upstream ещё не выпустил stable и есть
  ...
  явной пометкой в progress-документе и задачей на апгрейд при
  первом stable-релизе.

Следствие:

* любой code change, который вводит или меняет version pin, обязан
  сопровождаться sync-ом progress-документа;
...
Он должен запускаться только после того, как в local harness уже есть минимально рабочий host/LXD substrate через реальные роли `base_system`, `binary_fetch`, `lxd_snap`, `lxd_project`, `lxd_storage_pools`, `lxd_network_ext_bridge`, `lxd_network_int_managed`, `lxd_profiles`.
...
Общее обязательное правило для всех ролей этого repo:

* роли должны быть максимально declarative;
* использование shell-based task execution запрещено везде, где доступны native Ansible modules или modules из collections;
* shell fallback допустим только как документированное исключение последней инстанции;
* каждая роль должна иметь prefixed defaults, dispatcher-only `tasks/main.yml`, отдельный `preflight.yml` для input validation и короткие тематические task files;
* имена tasks/handlers обязаны следовать схеме `<role> | <section> | <action>`;
* каждая роль обязана иметь `README.md` с публичным contract, а не только код;
* роли этого repo владеют только host/bootstrap/harness layer и не должны становиться владельцем cluster add-ons или guest networking для CAPN-managed nodes.
...
## 8.6. `lxd_network_ext_bridge`

Делает:

* configure `br-ext6`;
* attach uplink;
* ensure bridge/firewall sane;
* no interference with existing unrelated bridges

## 8.7. `lxd_network_int_managed`
...
* поднимает validation path поверх уже существующего local substrate;
* использует те же network roles и profiles, что и основной implementation path;
* проверяет RA/NDP/multiple-MAC/inbound IPv6 без отдельного параллельного дизайна.
...
## 8.16. `pivot_clusterctl_move`

Только если `pivot_enabled=true`:
...
## 9.1. `modules/capi_cluster_class`
...
## 9.2. `modules/capi_lxc_templates`
...
## 9.3. `modules/capi_management_cluster`

Содержит:

* target self-hosted management cluster
* минимум 1 worker node
...
## 9.4. `modules/capi_workload_cluster`
...
## 9.5. `modules/cluster_addons_helm`
...
## 9.6. Test root modules

В этом repo root modules допускаются **только как test fixtures** под локальный harness.

### `tests/fixtures/terraform/management-cluster/capi`

Используется только если `pivot_enabled=true` в local e2e или integration test.
Provider:
...
Эти root modules нужны, чтобы верифицировать reusable `modules/*` в локальном контуре. Реальные environment root modules должны жить в отдельных private repos.
...
# 10. Типизированный контракт переменных
...
host:
  ...
  lxd_snap_channel: {type: string, default: "6/stable"}          # plan §2.11 — newest feature-stable track
  lxd_snap_refresh_mode: {type: string, default: "hold"}         # hold|timer
  lxd_snap_refresh_timer: {type: string, default: "fri,03:00-04:00"}
...
bootstrap:
  ...
  # Defaults track current upstream stable per plan §2.11; every bump must be
  # verified via upstream releases API and recorded in PLAN-stage1-progress.md.
  k3s_version: {type: string, default: "v1.35.3+k3s1"}
  kubectl_version: {type: string, default: "v1.35.3"}
  clusterctl_version: {type: string, default: "v1.12.5"}
  capn_provider_version: {type: string, default: "v0.8.5"}
...
addons:
  # Defaults track current upstream stable per plan §2.11; see progress doc
  # for "Версии зафиксированы" table with verification dates.
  helm_provider_version: {type: string, default: "3.1.1"}
  ...
  flannel_chart_version: {type: string, default: "v0.28.4"}
  ...
  calico_chart_version: {type: string, default: "v3.31.5"}
  ...
  metallb_chart_version: {type: string, default: "0.15.3"}
...
clusters:
  ...
  kubernetes_version: {type: string, default: "v1.35.3"}
...
# 11. Локальная разработка и тестирование

## 11.1. Базовый локальный контур
...
## 11.2. Как смоделировать второй интерфейс и IPv6 /64
...
Это даст вам **mocked RA/DHCPv6 /64**, который увидит второй NIC Debian test VM. Затем уже **внутри самой VM** роль `lxd_network_ext_bridge` создаст `br-ext6` и привяжет к нему этот второй NIC так, как это будет требоваться consumer-окружению. ([Libvirt][23])

## 11.3. Как проверить внешний ingress локально
...
## 11.4. Что должно тестироваться в Molecule
...
# 12. Порядок реализации для coding agents

Ниже — **единственный правильный порядок**.

## Phase 0 — repo skeleton и local harness
...
* Btrfs pool exists
* internal managed network exists
* external bridge exists
* no damage to foreign containers

## Phase 2.5 — External L2 viability gate

Сделать:

* `ext6-mock` libvirt network
* test VM with second NIC
* внутри VM `br-ext6` через ту же reusable роль
* 2 test LXC on `br-ext6` через тот же LXD substrate path
* validation orchestration only via `gate_ext_l2`

Acceptance:

* multiple MAC pass
* RA visible inside LXC
* NDP works
* inbound IPv6 works from probe endpoint
...
## Phase 5 — Terraform against bootstrap cluster
...
## Phase 5.05 — Export target kubeconfig
...
## Phase 6 — Pivot (optional)

Only when `pivot_enabled=true`:
...
# 13. One-command local workflows
...
## 16.1. External L2 may fail

Mitigation:

* Phase 2.5 gate before дальнейшим продвижением по cluster path

## 16.2. Unprivileged LXC node path may fail on userns/runtime/CNI edges
...
## 16.3. Default route may land on external NIC
...
## 16.4. kubelet may pick wrong node IP
...
## 16.5. Snap auto-refresh may destabilize LXD
...
## 16.6. Stage 2 pivot adds complexity without HA gain

Mitigation:

* `pivot_enabled=false` by default

## 16.7. CAPN + Canonical LXD drift risk
...
## 16.8. Secrets leakage through artifacts
...
## 16.9. CAPN pre-built images are evaluation-oriented
...
# 17. Саморевью: что не потеряно из исходного контракта
...
* Debian 13;
* Btrfs pool with path-based source;
...
* accepted review fixes: external L2 gate, CNI gate, destroy contract, secrets story, LXD API path, CAPN profile baseline, optional pivot, typed vars, out-of-scope section, snap refresh policy.
...
* gate-фазы выполняются на том же reusable implementation path, а не через отдельные research spikes.
...
3. затем сделать **host bootstrap** и **LXD substrate**;
4. после этого пройти **Phase 2.5 external L2 gate** на том же reusable path;
5. потом собрать **bootstrap cluster**, применить **Terraform CAPI fixture**, экспортировать kubeconfig target cluster и только затем применить **Terraform Helm add-ons fixture**;
6. на первом Terraform-created cluster пройти **Phase 5.2 CNI gate** и **Phase 5.3 MetalLB smoke**;
7. только потом идти в optional pivot / post-pivot workload path;
8. MVP считать готовым, когда:
