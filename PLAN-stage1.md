...
* **A role MUST NOT read or condition on variables using another
  ...
  только через:
  1. global contract переменные без role-префикса (§10, типа
...
host:
  ...
  # host.* lxd_host_* inputs are consumed by role lxd_host (plan §8.3).
  lxd_host_snap_channel: {type: string, default: "6/stable"}          # plan §2.11 — newest feature-stable track
...
* `base_system`
* `lxd_host`
* `binary_fetch` (отодвинут ближе к Phase 4 — kubectl/clusterctl/k3s не потребляются раньше)
...
## Phase 2 — LXD substrate (entities inside LXD)
